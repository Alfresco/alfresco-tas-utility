...
public class DataLDAP
{
    ...
    public enum UserAccountControlValue{
        ...
        UserAccountControlValue(String userAccountControlValue){this.userAccountControlValue= userAccountControlValue; }

        @Override
        public String toString(){ return userAccountControlValue;}
    }
    ...
    public class Builder implements UserManageable, GroupManageable
    {
        ...
        public Builder disableUser(UserModel user) throws NamingException {
            Attribute memberAttribute = new BasicAttribute("userAccountControl", Integer.toString(UserAccountStatus.ACCOUNTDISABLE.getValue() + UserAccountStatus.NORMAL_ACCOUNT.getValue() + UserAccountStatus.PASSWD_NOTREQD.getValue()));
        ...
        public Builder enableUser(UserModel user) throws NamingException {
            Attribute memberAttribute = new BasicAttribute("userAccountControl", Integer.toString(UserAccountStatus.NORMAL_ACCOUNT.getValue() + UserAccountStatus.PASSWD_NOTREQD.getValue()));
            ModificationItem modification[]= new ModificationItem[1];
            modification[0] = new ModificationItem(DirContext.REPLACE_ATTRIBUTE, memberAttribute);
            context.modifyAttributes(String.format(USER_GROUP_SEARCH_BASE, user.getUsername()),modification);
            return this;
        ...
        public Builder createEnabledUserPasswordNotRequired(UserModel user) throws NamingException
        {
            ...
            userPassword.add(user.getPassword());
            userAccountControl.add(Integer.toString(UserAccountStatus.NORMAL_ACCOUNT.getValue() + UserAccountStatus.PASSWD_NOTREQD.getValue() + UserAccountStatus.PASSWORD_EXPIRED.getValue()));
    ...
    public Builder createDisabledUser(UserModel user) throws NamingException
    {
        ...
        public Builder assertUserIsDisabled(UserModel user, UserAccountControlValue userAccountControlValue) throws NamingException {
            ...
            Assert.assertTrue(accountStatus.toString().contains(userAccountControlValue.toString()), String.format("User account control value expected %s but found %s", userAccountControlValue.toString(), accountStatus.toString()));
            return this;
        }

        public Builder assertUserIsEnabled(UserModel user, UserAccountControlValue userAccountControlValue) throws NamingException {
            Attributes accountStatus = context.getAttributes(String.format(USER_GROUP_SEARCH_BASE, user.getUsername()), new String []{"userAccountControl"});
            Assert.assertTrue(accountStatus.toString().contains(userAccountControlValue.toString()), String.format("User account value expected %s but found %s ", userAccountControlValue.toString(), accountStatus.toString()));
            return this;
        }
        ...
        public GroupManageable assertUserIsMemberOfGroup(UserModel user, GroupModel group) throws NamingException
        {
            ...
            Assert.assertTrue(membership.toString().contains(String.format(USER_GROUP_SEARCH_BASE, user.getUsername())), String.format("User %s is not member of group %s",user.getUsername().toString(), group.getDisplayName().toString()));
            return this;
        }
        ...
        {
            Attributes membership  = context.getAttributes(String.format(USER_GROUP_SEARCH_BASE, group.getDisplayName()), new String[]{"member"});
            Assert.assertFalse(membership.toString().contains(String.format(USER_GROUP_SEARCH_BASE, user.getUsername())), String.format("User %s is member of group %s",user.getUsername().toString(), group.getDisplayName().toString()));
            return this;
        }
    }
}
